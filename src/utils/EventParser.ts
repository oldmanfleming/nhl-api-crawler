        ...
        case EventTypeId.BlockedShot: {
          ...
          for (const homePlayerId of homePlayers) {
            if (homePlayerId !== blockedShotEvent.playerId && homePlayerId !== shotBlockedEvent.playerId)
              events.push(
                getEvent(blockedShotEvent.isHome ? EventType.OnIceBlockedShot : EventType.OnIceShotBlocked, homePlayerId, metaInfo)
              )
          }
          for (const awayPlayerId of awayPlayers) {
            if (awayPlayerId !== blockedShotEvent.playerId && awayPlayerId !== shotBlockedEvent.playerId)
              events.push(
                getEvent(blockedShotEvent.isHome ? EventType.OnIceShotBlocked : EventType.OnIceBlockedShot, awayPlayerId, metaInfo)
              )
          }
          break
        }
        ...
        case EventTypeId.Penalty: {
          ...
          for (const homePlayerId of homePlayers) {
            if (homePlayerId !== penaltyAgainstEvent.playerId && (!penaltyForEvent || penaltyForEvent.playerId !== homePlayerId))
              events.push(
                getEvent(penaltyAgainstEvent.isHome ? EventType.OnIcePenaltyAgainst : EventType.OnIcePenaltyFor, homePlayerId, metaInfo)
              )
          }
          for (const awayPlayerId of awayPlayers) {
            if (awayPlayerId !== penaltyAgainstEvent.playerId && (!penaltyForEvent || penaltyForEvent.playerId !== awayPlayerId))
              events.push(
                getEvent(penaltyAgainstEvent.isHome ? EventType.OnIcePenaltyFor : EventType.OnIcePenaltyAgainst, awayPlayerId, metaInfo)
              )
          }
          break
        }
